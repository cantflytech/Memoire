  ...
  <main class="heading min-h-screen bg-[#F8FAFB] font-['Inter'] flex items-center justify-center">
    <div class="max-w-7xl w-full bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden mt-12">
      ...
      <footer class="p-6 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
        ...
        <button 
          @click="activeLesson < 6 ? activeLesson++ : handleFinishLesson()"
          :disabled="finishingLoading"
          class="px-5 py-2.5 rounded-xl bg-[#00AA90] hover:bg-[#008F7A] text-white text-xs font-black shadow-xs transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {{ finishingLoading ? 'Validation...' : (activeLesson === 6 ? 'Terminer le cours ✓' : 'Continuer →') }}
        </button>
      </footer>
