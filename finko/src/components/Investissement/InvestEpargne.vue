...
<template>
  <div class="body max-w-7xl mx-auto w-full font-['Inter']">
    ...
    <div v-else class="background-color-purple text-white rounded-[20px] p-5 md:p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-4 relative overflow-hidden">
      ...
      <div class="flex flex-col items-center sm:items-start z-10 w-full sm:w-auto text-center sm:text-left">
        <div class="px-2.5 py-1 rounded-md flex items-center gap-1.5 mb-2">
          <span class="text-xs"><svg class="w-6 h-6 text-[#C0C1F9]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path>
</svg></span>
          <span class="heading text-xl opacity-90">Mode croissance</span>
        </div>
        
        <div class="flex flex-col">
          <span class="text-3xl md:text-4xl font-black heading tracking-tight">
            {{ investment.toLocaleString()}} <span class="text-xl md:text-2xl font-bold">€</span>
          </span>
